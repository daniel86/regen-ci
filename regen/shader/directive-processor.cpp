...
void DirectiveProcessor::MacroBranch::add(bool isDefined) {
	...
	parent->childs_.emplace_back(defined, parent);
}
...
void DirectiveProcessor::MacroTree::_undef(const string &s) { defines_.erase(s); }

void DirectiveProcessor::MacroTree::_ifdef(const string &s) { root_.open(evaluate(s)); }

void DirectiveProcessor::MacroTree::_ifndef(const string &s) { root_.open(!evaluate(s)); }

void DirectiveProcessor::MacroTree::_if(const string &s) { root_.open(evaluate(s)); }
...
bool DirectiveProcessor::MacroTree::isDefined() { return root_.getActive().isDefined_; }
...
bool DirectiveProcessor::process(PreProcessorState &state, string &line) {
	...
	if (hasPrefix(statement, "#line ")) {
		...
		boost::trim(key);
		string imported;
		auto needle = state.in.externFunctions.find(key);
		...
		if (imported.empty()) {
			...
			inputs_.emplace_front(ref_ptr<StreamProcessor>::alloc(imported));
		}
		return DirectiveProcessor::getline(state, line);
