...
namespace regen {
	...
	struct PatchLevels {
		 ...
		 * @return false if values are component-wise equal
		 */
		inline bool operator!=(const PatchLevels &b) const { return inner_ != b.inner_ || outer_ != b.outer_; }
	};
	...
	struct TextureBind {
		 ...
		 * @return false if values are component-wise equal
		 */
		inline bool operator!=(const TextureBind &b) const { return id_ != b.id_ || target_ != b.target_; }
	};
	...
	struct BufferRange {
		...
		/**
		 * \brief Returns a null reference buffer range.
		 */
	...
	public:
		...
		inline void beginTransformFeedback(GLenum v) {
			if (feedbackCount_ == 0) { glBeginTransformFeedback(v); }
			feedbackCount_ += 1;
		}
		...
		inline void endTransformFeedback() {
			feedbackCount_ -= 1;
			if (feedbackCount_ == 0) { glEndTransformFeedback(); }
		}
