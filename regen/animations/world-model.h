...
namespace regen {
	...
	struct Affordance {
		...
		ref_ptr<WorldObject> target; // optional target object
	};

	class WorldObject {
	public:
		WorldObject(std::string_view name, const Vec2f &position) :
			name_(name),
			pos_(createUniform<ShaderInput3f>("pos", Vec3f(position.x, 0.0f, position.y))) {}
		...
		void addAffordance(
				AffordanceType type,
				float minDistance,
				const ref_ptr<WorldObject> &target={}) {
			ref_ptr<Affordance> aff = ref_ptr<Affordance>::alloc();
			aff->type = type;
			aff->minDistance = minDistance;
			aff->target = target;
			affordances_.push_back(aff);
		}
	...
	public:
		WayPoint(std::string_view name, const Vec2f &position) :
	 ...
	 */
	class Place : public WayPoint {
	public:
		Place(std::string_view name, PlaceType type) :
			WayPoint(name, Vec3f::zero()),
			type(type) {}
		Place(std::string_view name, PlaceType type, const Vec2f &position) :
